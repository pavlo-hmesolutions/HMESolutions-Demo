...
class ResourceEvents extends Component {
  ...
  render() {
    ...
    const eventContainer = (
      <div ref={this.eventContainerRef} className="event-container" style={{ height: '60px' }}>
        {selectedArea}
        {eventList}
      </div>
    );
    return (
      <tr style={{height: '60px'}}>
        <td style={{ width: rowWidth, height: '60px' }}>{config.dragAndDropEnabled ? connectDropTarget(eventContainer) : eventContainer}</td>
      </tr>
    );
  }
}
